...
const envSchema = z.object({
  ...
  APP_URL: z
    .string()
    .url({ message: "APP_URL must be a valid URL (e.g., http://localhost:3000)." }),
  DATABASE_URL: z
    .string()
  ...
  NEXTAUTH_URL: z
    .string()
    .url({ message: "NEXTAUTH_URL must be a valid URL (e.g., http://localhost:3000)." }),
  ...
  THIRD_PARTY_API_KEY: z
    .string()
    .min(1, "THIRD_PARTY_API_KEY must be provided for third-party integrations."),
  THIRD_PARTY_API_SECRET: z.string().min(1, "THIRD_PARTY_API_SECRET is required."),
